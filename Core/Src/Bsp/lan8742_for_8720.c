...
/**
 ...
 int32_t LAN8742_Init(lan8742_Object_t *pObj)
 {
   ...
   if(pObj->Is_Initialized == 0)
   {
     ...
     /* 软复位 */
     if(pObj->IO.WriteReg(addr, LAN8742_BCR, LAN8742_BCR_SOFT_RESET) < 0){
    	 oledLogClear();
    	 oledLog("In LAN8742_Init soft reset: write error");
    	 return LAN8742_STATUS_WRITE_ERROR;
     }
     do {
    	 if (pObj->IO.ReadReg(addr, LAN8742_BCR, &regvalue) < 0) {
    		 oledLogClear();
    		 oledLog("In LAN8742_Init soft reset: read error");
    		 return LAN8742_STATUS_READ_ERROR;
    	 }
     } while(regvalue & LAN8742_BCR_SOFT_RESET);  // 等待软复位完成
     ...
     if (pObj->IO.WriteReg(addr, LAN8742_BCR, regvalue) < 0) {
    	 return LAN8742_STATUS_WRITE_ERROR;
     }
...
int32_t LAN8742_GetLinkState(lan8742_Object_t *pObj)
{
  ...
  {
    if(((readval & LAN8742_BCR_SPEED_SELECT) == LAN8742_BCR_SPEED_SELECT) && ((readval & LAN8742_BCR_DUPLEX_MODE) == LAN8742_BCR_DUPLEX_MODE))  // 判断指定位是否为一
    {
