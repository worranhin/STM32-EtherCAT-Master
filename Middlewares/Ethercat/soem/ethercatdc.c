...
boolean ecx_configdc(ecx_contextt *context)
{
   ...
   ecx_BWR(context->port, 0, ECT_REG_DCTIME0, sizeof(ht), &ht, EC_TIMEOUTRET);  /* ���ʹ����ӳٱ��ģ�����˿�ʱ�� */
   ...
   for (i = 1; i <= *(context->slavecount); i++)
   {
      ...
      if (context->slavelist[i].hasdc)
      {
         ...
         (void)ecx_FPRD(context->port, slaveh, ECT_REG_DCTIME0, sizeof(ht), &ht, EC_TIMEOUTRET);//��ȡ���˿ڵ������ʱ��
         context->slavelist[i].DCrtA = etohl(ht);
         /* 64bit latched DCrecvTimeA of each specific slave */
         (void)ecx_FPRD(context->port, slaveh, ECT_REG_DCSOF, sizeof(hrt), &hrt, EC_TIMEOUTRET);//��ȡ���˿ڵ������ʱ��
         ...
         (void)ecx_FPWR(context->port, slaveh, ECT_REG_DCSYSOFFSET, sizeof(hrt), &hrt, EC_TIMEOUTRET);//д��ʱ��ƫ����
