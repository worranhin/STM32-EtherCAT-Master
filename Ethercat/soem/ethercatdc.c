...
boolean ecx_configdc(ecx_contextt *context)
{
   ...
   ecx_BWR(context->port, 0, ECT_REG_DCTIME0, sizeof(ht), &ht, EC_TIMEOUTRET);  /* 发送传输延迟报文，所存端口时间 */
   ...
   for (i = 1; i <= *(context->slavecount); i++)
   {
      ...
      if (context->slavelist[i].hasdc)
      {
         ...
         (void)ecx_FPRD(context->port, slaveh, ECT_REG_DCTIME0, sizeof(ht), &ht, EC_TIMEOUTRET);//读取各端口的所存的时间
         context->slavelist[i].DCrtA = etohl(ht);
         /* 64bit latched DCrecvTimeA of each specific slave */
         (void)ecx_FPRD(context->port, slaveh, ECT_REG_DCSOF, sizeof(hrt), &hrt, EC_TIMEOUTRET);//读取各端口的所存的时间
         ...
         (void)ecx_FPWR(context->port, slaveh, ECT_REG_DCSYSOFFSET, sizeof(hrt), &hrt, EC_TIMEOUTRET);//写入时间偏移量
